...
Перед ежедневной тарификацией backend сохраняет снимок количества всех найденных
профилей пользователя. Первый профиль входит в базовую стоимость, а каждый следующий
добавляет 50% базового суточного списания. Если свежий снимок 3X-UI получить не удалось,
тарификация текущего дня откладывается и повторяется без частичного списания.
...
## Telegram-уведомления о балансе

Backend может уведомлять пользователей через внешний API Telegram-бота при переходе
баланса в минус, приближении к допустимому пределу и автоматической billing-блокировке.
Telegram User ID задаётся только администратором в настройках пользователя и не входит
в пользовательские ответы API.

Для включения интеграции задайте обе переменные:

```dotenv
TG_NOTIFICATION_URL=https://notifications.example.com/send
TG_NOTIFICATION_TOKEN=replace_with_notification_api_token
```

Backend отправляет на этот URL `POST` с Bearer-токеном и JSON
`{"tgid": 123456789, "text": "..."}`. Если обе переменные пусты, интеграция отключена.
Если задана только одна из них, backend завершит запуск с ошибкой конфигурации. Ошибка
отправки уведомления записывается в лог, не отменяет тарификацию и не повторяется.
